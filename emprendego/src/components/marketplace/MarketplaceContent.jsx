...
// ============================================
// COMPONENTE PRINCIPAL - MarketplaceContent
// ============================================

/**
 * MarketplaceContent - Contenido principal del marketplace
 * 
 * Este componente puede ser usado:
 * 1. Como página completa (MarketplaceHome)
 * 2. Como sección embebida en el landing (MarketplaceSection)
 * 
 * @param {boolean} embedded - Si está embebido en otra página (sin hero)
 * @param {boolean} showHero - Mostrar sección hero
 * @param {boolean} showHowItWorks - Mostrar sección "Cómo funciona"
 * @param {boolean} showCta - Mostrar CTA "Vende con nosotros"
 * @param {number} maxStoresInGrid - Máximo de tiendas a mostrar en modo embebido
 */
export default function MarketplaceContent({
  ...
  // Paginación
  const itemsPerPage = maxStoresInGrid || ITEMS_PER_PAGE
  const totalPages = Math.ceil(finalFilteredStores.length / itemsPerPage)
  ...
  if (!loading && !marketplaceEnabled) {
    return (
      <div className={`${embedded ? 'py-12' : 'min-h-screen'} bg-gradient-to-br from-slate-50 to-slate-100 flex items-center justify-center p-4`}>
  ...
  return (
    <div className={embedded ? '' : 'min-h-screen bg-white'}>
      {/* ============================================ */}
      {/* HERO (solo si no está embebido y showHero) */}
      {/* ============================================ */}
      {!embedded && showHero && (
          ...
          <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24">
            <div className="text-center max-w-3xl mx-auto">
              ...
              <p className="text-lg md:text-xl text-slate-600 mb-8 max-w-2xl mx-auto">
                Explora tiendas digitales de emprendedores locales.
                Conecta con negocios de tu ciudad y apoya el comercio local.
              </p>
      ...
      {/* ============================================ */}
      {/* CÓMO FUNCIONA (solo si showHowItWorks y no embebido) */}
      {/* ============================================ */}
      {!embedded && showHowItWorks && (
      ...
      {!loading && featuredStores.length > 0 && (
        <section className={`py-12 md:py-16 ${embedded ? 'bg-gradient-to-br from-amber-50/50 to-orange-50/50' : 'bg-gradient-to-br from-amber-50 to-orange-50'}`}>
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      {/* BUSCADOR Y FILTROS */}
      {/* ============================================ */}
      <section id="tiendas" className={`py-12 md:py-16 ${embedded ? 'bg-white' : 'bg-slate-50'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {/* Título */}
          <div className="text-center mb-8">
            <h2 className="text-3xl font-bold text-slate-900 mb-3">
              {embedded ? 'Explora el Marketplace' : 'Explora todas las tiendas'}
            </h2>
            <p className="text-slate-600">
              Encuentra lo que buscas entre {stores.length} tiendas activas
            </p>
          </div>
          ...
          {loading ? (
            ...
            <>
              ...
              {/* Link "Ver más" para modo embebido */}
              {embedded && finalFilteredStores.length > itemsPerPage && (
      ...
      {/* ============================================ */}
      {/* CTA - VENDE CON NOSOTROS (solo si showCta y no embebido) */}
      {/* ============================================ */}
      {!embedded && showCta && (
...
// Exportar componentes auxiliares para uso externo
